      ...
      steps {
        script {
          ...
          def currentVersion = sh(
            script: "python3 -c \"import re; match = re.search(r'version=\\\\'(.*?)\\\\'', open('setup.py').read()); print(match.group(1) if match else 'NOT FOUND')\"",
              returnStdout: true
              ).trim()
      ...
      steps {
        script {
          ...
          dir('AuroPro_Project_3'){
            ...
            echo "${envPrefix}"
            sh "terraform init"
            sh "terraform workspace new ${envPrefix} || terraform workspace select ${envPrefix}"
            sh "terraform plan"
